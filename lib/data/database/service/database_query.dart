...
class DatabaseQuery {
  ...
  Future<int> createRule(String rule, String ruleTranslation, String ruleDescription, String ruleDescriptionTranslation,  String example, String additionalComment) async {
    var dbClient = await databaseService.db;
    final ruleData = {'rule': rule, 'rule_translation': ruleTranslation, 'rule_description': ruleDescription, 'rule_description_translation': ruleDescriptionTranslation, 'example': example, 'additional_comment': additionalComment};
    return await dbClient.insert('Table_of_rules', ruleData, conflictAlgorithm: sql.ConflictAlgorithm.replace);
  }

  Future<int> updateRule(int ruleId, String rule,  String ruleTranslation, String ruleDescription, String ruleDescriptionTranslation, String example, String additionalComment) async {
    var dbClient = await databaseService.db;
    final ruleNewData = {'rule': rule, 'rule_translation': ruleTranslation, 'rule_description': ruleDescription, 'rule_description_translation': ruleDescriptionTranslation, 'example': example, 'additional_comment': additionalComment};
